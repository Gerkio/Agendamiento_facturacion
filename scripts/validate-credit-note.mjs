...
if (!xsd.valid) xsd.errors.slice(0,12).forEach(e => console.log('  •', styleText('red', e)))

const sch = await validateBusinessRules(signed)
console.log('\n=== Schematron (reglas de negocio) ===')
console.log('VÁLIDO:', sch.valid ? styleText('green', 'true') : styleText('red', 'false'), '| relevantes:', sch.findings.length, '| ignoradas:', sch.ignored.length)
for (const f of sch.findings) console.log(styleText('red', `  ❌ [${f.id}] ${f.text}`))
for (const f of sch.ignored) console.log(styleText('dim', `  ⚪ [${f.id}] (ignorada)`))
