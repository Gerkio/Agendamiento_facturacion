...
\#\#\#\# CLÁUSULA SEGUNDA: ALCANCE DEL TRABAJO Y ENTREGABLES (MVP)  
\*\*EL DESARROLLADOR\*\* ejecutará el proyecto estructurándolo en los siguientes módulos funcionales exclusivos para el uso empresarial interno de \*\*EL CONTRATANTE\*\*:

1\. \*\*Módulo de Directorio Operativo (CRUD) y Migración:\*\* Construcción de una base de datos relacional en la nube para Clientes y Aseadoras. Incluye el proceso técnico de migración e importación inicial de la data histórica desde el archivo plano suministrado por \*\*EL CONTRATANTE\*\*.  
2\. \*\*Calendario Interactivo con Alertas (Matching Manual):\*\* Panel visual de administración para programar servicios individuales y recurrentes (quincenales/mensuales). El sistema evaluará algorítmicamente las asignaciones del administrador y emitirá una alerta visual en pantalla en caso de detectar colisiones o cruces de horarios para una misma trabajadora.  
3\. \*\*Interfaz de Consulta para Empleadas:\*\* Acceso web optimizado para dispositivos móviles, bajo modalidad exclusiva de lectura, que permitirá al personal operativo loguearse de forma segura y consultar su agenda diaria de turnos.  
4\. \*\*Módulo de Facturación Nativa DIAN (Costo $0 de API):\*\* Módulo interno que compilará los servicios realizados, calculará el Código Único de Facturación Electrónica (CUFE), firmará digitalmente los archivos XML y los transmitirá directamente vía Web Services SOAP UBL 2.1. Permitirá facturar por servicios individuales o emitir facturas consolidadas por lotes (quincenales o mensuales) para un mismo cliente.
...
4\. \*\*Restricciones de Explotación:\*\* Queda expresamente prohibido para \*\*EL CONTRATANTE\*\*, sus socios, empleados, contratistas o cualquier tercero vinculado, de forma directa o indirecta: revender, sublicenciar, distribuir, ceder, transferir, comercializar o prestar servicios de "software como servicio" (SaaS) a terceros utilizando la plataforma licenciada, así como someterla a procesos de ingeniería inversa, desensamblado o descompilación fuera de los límites autorizados por este instrumento.  
...
\*\*EL CONTRATANTE\*\*    
Representante Legal de \*\*AMARU S.A.S.\*\*    
